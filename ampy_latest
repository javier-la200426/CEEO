    ...
    async def upload(self, filename):
        ...
        with open('%s', 'rb') as infile:
            while True:
                ...
                if result == b'':
                    break
                len = sys.stdout.write(ubinascii.hexlify(result).decode())
        ...
        try:
            ...
            print('file sent')
            out = await self.read_until(1, '>') #this prints many numbers
            print('got it')
        ...
        return binascii.unhexlify(out) #this is the payload or the text that is in device
