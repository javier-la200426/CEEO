...
SEPARATOR = '>' #will enter a timeout if not '>'. This is because of this:
#change separtor if there is a filename or data_piece with '>' in it
...
It will read each piece of data until '!'
BUT there is no '!' after the EOF signifier '>'. Thus it will try to keep reading
...
class campy():
    ...
    #define a function that is called whenever a file is clicked
    ...
    async def List_files(self):
        ...
        command_0 = f"""
            ...
            for i in os.ilistdir():
                #data is sparated by '{SEPARATOR}'
                print(i[0], end = '{SEPARATOR}') #filename
                print(i[1], end = '{SEPARATOR}') #directory is 16384 and file is 32768
                print(i[2], end = '{SEPARATOR}') #inode of file
                print(i[3], end = '{SEPARATOR}') #file size
        """
        ...
        #Thus, we convert object to string usin json.
        #we pass string to javascript function
        await js.displayFiles(json.dumps(my_files)) #display the files
    ...
    async def store_file_info(self):
        ...
        # that is exactly '>', indicating that you have reached the end
        # This is because the end looks like this slfjlksdfjl...>>
        while True:
            #print("1")
            data_piece = await self.read_until(1, SEPARATOR, timeout = 50) # Reads output after executing command, which should be a number
            print(data_piece)
            if (data_piece == ">"):
                print("EXITIING")
                break
