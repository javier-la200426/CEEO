...
async def handle_board(event):
    ...
    if event.type == 'mpy-run':
        ...
        if my_globals.terminal.connected:
            ...
            code_og = event.detail.code
            #add async.sleep to all loops in code
            add_async_code = add_async_sleep_to_loops(code_og)

            await my_globals.terminal.eval('\x05' + code_og + "#**END-CODE**#" + '\x04')
...
def add_async_sleep_to_loops(code):
    """
    Adds 'await asyncio.sleep(0.01)' to all loops in the given code.

    Args:
        code (str): The input code as a string.

    Returns:
        str: The modified code with 'await asyncio.sleep(0.01)' added to all loops.
    """

    #add asyncio if not already on code
    ...
    for line in lines:
        ...
        if in_loop and stripped_line == '':
            in_loop = False

        result.append(line)

        if in_loop and (stripped_line.startswith('for ') or stripped_line.startswith('while ') or len(stripped_line) == 0):
            # Adding the sleep line
            sleep_line = ' ' * (indent_level + 4) + 'await asyncio.sleep(0.01)'
            result.append(sleep_line)
        
        # Detect the end of the loop
        if in_loop and len(line) - len(stripped_line) <= indent_level:
            in_loop = False
