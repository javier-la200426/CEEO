...
#chunk is acting like data in the other command2
    ...
    async def simple_hash_3(self, chunk):
        hash_value = 0
        for byte in chunk:
            hash_value = (hash_value * 31 + byte) % (2**99)
        return hash_value
    ...
    - Accumulates the hash value using the formula (hash_value * 31 + await self.simple_hash_3(chunk)) % (2**99).
    ...
    async def hashfile(self, all_text):
        ...
        for chunk_start in range(0, len(bytes_data), BUFFER_SIZE):
            ...
            hash_value = (hash_value * 31 + await self.simple_hash_3(chunk)) % (2**99)
